...
mender_merge_bootfs_and_image_boot_files() {
    ...
    # --> entries in MENDER_IMAGE_BOOT_FILES_REMOVE
    #     are removed from vfat /uboot
    mender_image_boot_files_remove="${MENDER_IMAGE_BOOT_FILES_REMOVE}"
    for entry in $mender_image_boot_files_remove; do
        image_boot_files=$(echo "$image_boot_files" | sed -e "s/$entry//")
    done
    # <-- entries in MENDER_IMAGE_BOOT_FILES_REMOVE
    #     are removed from vfat /uboot
   ...
   # --> add dummy file if partition is empty
   if [ "x$(printf '%s' "$image_boot_files" | tr -d "$IFS")" = x ]; then
      # if $image_boot_files is empty including space, tab, newline
      dummy="${W}/dummy.txt"

    cat >> "$dummy" <<EOF
# I am only some dummy file since there needs to be a file on vfat /uboot - no idea why ;)
EOF
    fi
    # <-- add dummy file if partition is empty
    # --> add uboot.env into vfat /uboot partition
    #install -m 0644 "${DEPLOY_DIR_IMAGE}/uboot.env" "${W}/"
    # <-- add uboot.env into vfat /uboot partition
}
...
mender_part_image() {
    ...
    if [ -n "${MENDER_IMAGE_BOOTLOADER_FILE}" ]; then
        ...
        cat >> "$wks" <<EOF
# embed bootloader - start at $bootloader_align_kb KBytes i.e. offset ($bootloader_align_kb * 1024 Bytes)
part --source rawcopy --sourceparams="file=$bootloader_file" --ondisk "$ondisk_dev" --align $bootloader_align_kb --no-table
EOF
    fi

# --> allow not only SPL but also u-boot.img to be in a raw partition
    if [ -n "${MENDER_IMAGE_SECOND_BOOTLOADER_FILE}" ]; then
        # Copy the files to embed in the WIC image into ${WORKDIR} for exclusive access
        install -m 0644 "${DEPLOY_DIR_IMAGE}/${MENDER_IMAGE_SECOND_BOOTLOADER_FILE}" "${WORKDIR}/"

        if [ $(expr ${MENDER_IMAGE_SECOND_BOOTLOADER_BOOTSECTOR_OFFSET} % 2) -ne 0 ]; then
            # wic doesn't support fractions of kiB, so we need to do some tricks
            # when we are at an odd sector: Create a new bootloader file that
            # lacks the first 512 bytes, write that at the next even sector,
            # which coincides with a whole kiB, and then write the missing
            # sector manually afterwards.
            second_bootloader_sector=$(expr ${MENDER_IMAGE_SECOND_BOOTLOADER_BOOTSECTOR_OFFSET} + 1)
            second_bootloader_file=${WORKDIR}/${MENDER_IMAGE_SECOND_BOOTLOADER_FILE}-partial
            dd if=${WORKDIR}/${MENDER_IMAGE_SECOND_BOOTLOADER_FILE} of=$second_bootloader_file skip=1
        else
            second_bootloader_sector=${MENDER_IMAGE_SECOND_BOOTLOADER_BOOTSECTOR_OFFSET}
            second_bootloader_file=${WORKDIR}/${MENDER_IMAGE_SECOND_BOOTLOADER_FILE}
        fi
        second_bootloader_align_kb=$(expr $(expr $second_bootloader_sector \* 512) / 1024)
        second_bootloader_size=$(stat -c '%s' "$second_bootloader_file")
        second_bootloader_end=$(expr $second_bootloader_align_kb \* 1024 + $second_bootloader_size)
        if [ $second_bootloader_end -gt ${MENDER_UBOOT_ENV_STORAGE_DEVICE_OFFSET} ]; then
            bberror "Size of second bootloader specified in MENDER_IMAGE_SECOND_BOOTLOADER_FILE" \
                    "exceeds MENDER_UBOOT_ENV_STORAGE_DEVICE_OFFSET, which is" \
                    "reserved for U-Boot environment storage. Please raise it" \
                    "manually."
        fi
        cat >> "$wks" <<EOF
# embed second bootloader - start at $second_bootloader_align_kb KBytes i.e. offset ($second_bootloader_align_kb * 1024 Bytes)
part --source rawcopy --sourceparams="file=$second_bootloader_file" --ondisk "$ondisk_dev" --align $second_bootloader_align_kb --no-table
EOF
    fi
# <-- allow not only SPL but also u-boot.img to be in a raw partition
